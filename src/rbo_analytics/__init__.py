...
def determine_K_p(probs, N=200):
    """Takes a list of probabilities  and determine using the elbow test
    an appropriate value of K, the size of the ranked list generated by the recommender,
    and then computes an appropriate value of the persistent parameter p via the 
    equation:

    $$ p^K <= 0.01, p = \log(0.01)/K $$

    Returns K & p.

    N determines how many entries of the sorted in descending probabilities we'll consider
    in determining the location of the elbow.
    """

    sorted_probs = np.sort(probs)[::-1][:N]

    K = find_elbow_f_test(sorted_probs)
    p = np.log(0.01)/K
